...
class MaxMaraWeekendAgent(RetailerAgent):
    RETAILER_NAME = "Max Mara Weekend"
    BASE_URL = "https://www.maxmara.com"
    ...
    async def search(
        ...
        category_paths = {
            "tops": "/us/women/tops",
            "dresses": "/us/women/dresses",
            "bottoms": "/us/women/pants-and-shorts",
            "outerwear": "/us/women/coats-and-jackets",
        }
        path = category_paths.get(category, "/us/women")
        url = f"{self.BASE_URL}{path}?collection=weekend&page={page_num}"
        ...
        for item in items:
            try:
                name_el = await item.query_selector(".product-title")
